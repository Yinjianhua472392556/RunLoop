//
//  RunloopLazyLoadViewController.m
//  RunLoopDemo
//
//  Created by 尹建华 on 2023/4/1.
//

#import "RunloopLazyLoadViewController.h"
...
@interface RunloopLazyLoadViewController ()<UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UITableView *exampleTableView;
@end

@implementation RunloopLazyLoadViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [RunloopLazyLoadViewController task_5:cell indexPath:indexPath];
    [RunloopLazyLoadViewController task_1:cell indexPath:indexPath];
    [[DWURunloopLazyLoad sharedRunloopLazyLoad] addTask:^BOOL(void) {
        if (![cell.currentIndexPath isEqual:indexPath]) {
            return NO;
        }
        [RunloopLazyLoadViewController task_2:cell indexPath:indexPath];
        return YES;
    } withKey:indexPath];
    [[DWURunloopLazyLoad sharedRunloopLazyLoad] addTask:^BOOL(void) {
        if (![cell.currentIndexPath isEqual:indexPath]) {
        ...
        [RunloopLazyLoadViewController task_3:cell indexPath:indexPath];
        return YES;
    } withKey:indexPath];
    [[DWURunloopLazyLoad sharedRunloopLazyLoad] addTask:^BOOL(void) {
        if (![cell.currentIndexPath isEqual:indexPath]) {
        ...
        [RunloopLazyLoadViewController task_4:cell indexPath:indexPath];
        return YES;
    } withKey:indexPath];
    return cell;
}
